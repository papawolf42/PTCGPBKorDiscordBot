...
GODPACK_WEBHOOK_CHANNEL_ID = os.getenv('DISCORD_GROUP7_DETECT_ID')
...
GROUP_CONFIGS = [
    {
        "NAME": "GROUP7",
        "HEARTBEAT_ID": int(os.getenv('DISCORD_GROUP7_HEARTBEAT_ID')), # Heartbeat (예시, 실제 그룹 ID에 맞게 조정 필요)
        "DETECT_ID": int(os.getenv('DISCORD_GROUP7_DETECT_ID')),
        "POSTING_ID": int(os.getenv('DISCORD_GROUP7_POSTING_ID')),
        "COMMAND_ID": int(os.getenv('DISCORD_GROUP7_COMMAND_ID')),
        "MUSEUM_ID": int(os.getenv('DISCORD_GROUP7_MUSEUM_ID')),
        "TAGS": {
...
async def process_heartbeat_message(message, channel_id_str, channel_name):
    ...
    try:
        ...
        # 그룹 이름 추출 ("Group7-Heartbeat" -> "Group7" 또는 채널 이름 그대로)
...
async def update_friend_lists(online_users_profiles):
    ...
    # --- 그룹별 배럭 현황 계산 및 출력 ---
    group_barracks = {"Group1": 0, "Group3": 0, "Group7": 0, "Other": 0, "Unknown": 0}
    ...
    for group, barracks in group_barracks.items():
        if barracks > 0 or group in ["Group1", "Group3", "Group7"]:
