...
- **에러 분석 → 근본 원인 설명**: 에러 발생 시 "왜" 일어났는지 상세히 설명
  - 단순히 에러 메시지만 보여주지 말고 원인 분석
  - 예: "PaddleOCR 초기화 실패" → "Apple Silicon에서 MKLDNN 지원 안 됨"
  - 가능한 해결 방법도 함께 제시
...
### 권장 작업 패턴
1. **코드 작성 전 계획 검토** ⭐⭐⭐ IMPORTANT NEW
   - 코드를 짜기 전에 먼저 계획을 사용자에게 설명
   - "이런 방향으로 진행하면 어떨까요?" 형식으로 제안
   - 다음 내용을 포함한 구체적인 계획 제시:
     * 수정할 파일 목록 (정확한 경로 포함)
     * 각 파일의 주요 변경 내용 요약
     * 변경이 영향을 미치는 범위 설명
     * 예상되는 부작용이나 주의사항
